...
class EvaluationPipeline:
    ...
    def _process_question_through_kb(self, query: str) -> Optional[str]:
        """
        Attempt to retrieve answer from Knowledge Base.
        """
        if not self.kb or not MODULES_AVAILABLE['kb']:
            return None
        
        try:
            results = self.kb.retrieve(query, k=3, threshold=0.1)
            ...
            logger.debug(f"KB retrieval error: {e}")
    ...
    def _generate_fallback_answer(self, query: str) -> str:
        ...
        if any(word in query_lower for word in ["tarif", "prix", "coût", "pricing", "grandes"]):
        ...
        elif any(word in query_lower for word in ["exporter", "export", "télécharger", "donnees", "data"]):
        ...
        elif any(word in query_lower for word in ["équipe", "team", "projet", "project", "configurer", "configuration"]):
        ...
        elif any(word in query_lower for word in ["sso", "connexion", "authentification", "single sign-on", "quoi"]):
    ...
    def process_questions_file(self, input_file: str) -> Dict[str, Any]:
        ...
        logger.info(f"✓ Processed {self.results['ProcessingMetadata']['successfully_processed']}/{len(questions)} questions")
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("Summary:")
    logger.info(f"  Total questions: {results['ProcessingMetadata']['total_questions']}")
    logger.info(f"  Successfully processed: {results['ProcessingMetadata']['successfully_processed']}")
