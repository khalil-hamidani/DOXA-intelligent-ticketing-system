...
def process_ticket(ticket: Ticket, team: str = None) -> Dict:
    ...
    if not v.get("valid"):
        ...
        return {"status": "invalid", "message": "Ticket invalide, merci de compléter le formulaire.", "reasons": v.get("reasons", []), "ticket": ticket}
    ...
    if eval_res.get("escalate"):
        ...
        return {"status": "escalated", "message": "Ticket escaladé vers un agent humain.", "escalation_context": eval_res.get("escalation_context"), "ticket": ticket}
